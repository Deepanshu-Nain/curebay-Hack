...
class Settings(BaseSettings):
    ...
    ALGORITHM: str = "HS256"
    ACCESS_TOKEN_EXPIRE_MINUTES: int = 43200  # 30 days (for ASHA workers offline for weeks)
    ...
    MEDGEMMA_N_GPU_LAYERS: int = 0      # 0 = CPU-only (safe for all devices)
    MEDGEMMA_TEMPERATURE: float = 0.2
    MEDGEMMA_MAX_TOKENS: int = 1500

    # ── Anthropic Claude API (fallback when MedGemma not downloaded) ──
    ANTHROPIC_API_KEY: str = ""          # Set in .env — enables Claude fallback
